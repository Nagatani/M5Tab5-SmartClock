...
void UINews::init(lv_obj_t* parent) {
    // 右側 45% (576 x 720 px, Xオフセット: 704px)
    _panel = lv_obj_create(parent);
    lv_obj_set_pos(_panel, CLOCK_PANEL_WIDTH, 0);
    lv_obj_set_size(_panel, NEWS_PANEL_WIDTH, SCREEN_HEIGHT);
    ...
    lv_obj_set_size(tabContainer, lv_pct(100), 55);
    ...
    for (int i = 0; i < CAT_MAX; ++i) {
        ...
        lv_obj_set_size(_btnTabs[i], 125, 45);
    ...
    lv_obj_set_size(_cardPanel, lv_pct(100), 480);
    lv_obj_align(_cardPanel, LV_ALIGN_TOP_MID, 0, 65);
    ...
    lv_obj_set_style_radius(_cardPanel, 12, 0);
    lv_obj_set_style_pad_all(_cardPanel, 16, 0);
    ...
    lv_obj_set_size(_barSlideProgress, lv_pct(100), 4);
    lv_obj_align_to(_barSlideProgress, _cardPanel, LV_ALIGN_OUT_BOTTOM_MID, 0, 8);
    ...
    lv_obj_set_size(bottomNav, lv_pct(100), 60);
    lv_obj_align(bottomNav, LV_ALIGN_BOTTOM_MID, 0, -10);
    ...
    lv_obj_set_size(_btnPrev, 100, 50);
    ...
    lv_obj_set_size(_btnSpeakNews, 300, 50);
    ...
    lv_obj_set_size(_btnNext, 100, 50);
...
void UINews::refreshCardView() {
    ...
    if (!cat.isLoaded || cat.itemCount == 0) {
        lv_label_set_text(_lblNewsTitle, "ニュースを取得中...");
        lv_label_set_text(_lblNewsDesc, "RSSフィードを受信しています。少々お待ちください。");
    ...
    lv_label_set_text(_lblNewsDesc, item.description.length() > 0 ? item.description.c_str() : "(本文要約なし)");
